...
$sayfa            	= @intval($_GET['s']);
					if(!$sayfa) $sayfa = 1;
$toplam           	= mysql_num_rows(mysql_query("select * from galeriresim"));
$limit            	= 2;
...
$gorunen        = 2;
    ...
    if($sayfa > 1){
       ...
       // $sayfalamaYaz.= "<a href='tumhaberler.html'>İlk</a>";
        $sayfalamaYaz.= '<li><a href="ana.php?git=galeriresimduzenle&id='.$id.'&s='.$onceki.'" class="btn prev prev_disabled"><span class="icon-chevron-left"></span></a></li>';
    ...
    if($sayfa != $sayfa_sayisi){
        $sonraki     = $sayfa +1;
        $sayfalamaYaz.= '<li><a class="btn next" href="ana.php?git=galeriresimduzenle&id='.$id.'&s='.$sonraki.'"><span class="icon-chevron-right"></span></a></li>';
...
<div class="breadcrumbwidget">
        	...
        	<ul class="breadcrumb">
                ...
                <li class="active">Galeri Resimleri</li>
            </ul>
        </div><!--breadcrumbs-->
        <div class="pagetitle">
        	<h1>Galeri resimlerini düzenleme</h1> <span>Bu Bölümden Galeri Kategorinize ait resimleri silebilir yada düzenleyebilirsiniz.</span>
        </div><!--pagetitle-->
      <?php
					$toplu	=$_POST["toplu"];
					$sil	=$_POST["sil"];
			if(isset($_POST["sil"])){
					foreach ($toplu as $toplusec){
						...
						$silme	=mysql_query("DELETE FROM galeriresim where grid='$toplusec'");
				
						
					}
						if($silme){
							uyari("Tüm seçilen resimler başarıyla silindi");
							git("ana.php?git=galeriresimduzenle&id=$id","3");
						}else{
    ...
    <i class="iconsweets-trashcan iconsweets-white" ></i>
                            ...
                            <li class="right newfilebtn"><a href="ana.php?git=galeriresim" class="btn btn-primary">Yeni resim ekle</a></li>
                        </ul>
                    ...
                    <div class="mediamgr_category">
                    	<ul>
							...
							<?php 
							$kac	=mysql_query("select * from galeriresim where galustid='$id'");
                    ...
                    <br /><br />
					     <ul class="listfile">
                    	<?php
								$dosyalar	=mysql_query("select * from galeriresim where galustid='$id' limit $goster,$limit");
									while($a=mysql_fetch_assoc($dosyalar)){
										$galerifoto		=$a["galerifoto"];
										$galeriaciklama	=$a["galeriaciklama"];
										$gridi			=$a["grid"];
						?>
					
                        	<li style="width:100px;height:130px;">
							<input type="checkbox" name="toplu[]" value="<?php echo $gridi;?>">
                                <a href="resimgit.php?resim=resimler&id=<?php echo $gridi;?>" data-rel="image" ><img src="<?php echo $galerifoto;?>" alt="" width="100" height="100"/></a>
