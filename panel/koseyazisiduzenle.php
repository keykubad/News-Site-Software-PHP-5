...
<div class="breadcrumbwidget">
        	...
        	<ul class="breadcrumb">
                ...
                <li class="active">Köşe yazısı düzenle</li>
            </ul>
        </div><!--breadcrumbwidget-->
      <div class="pagetitle">
        	<h1>Açıklama</h1> <span>Sitenize ait köşe yazılarını düzenleyebilirsiniz.</span>
        </div><!--pagetitle-->
		...
		if($_POST){
			...
			$hmanset				= $_FILES["hmanset"]["tmp_name"];
					...
					if($hmanset==""){
						...
						ustmanset='$ustmanset',sondakika='$sondakika',flash='$flash',yorumakapa='$yorumkapa',koseid='$koseyaz' where hid='$idsi'");
							if($haberekle){
								uyari("Köşe Yazısı Başarıyla Güncellendi!!");
								git("ana.php?git=koseyazisiduzenle&id=$idsi","1");
							}else{
								hata("Köşe Yazısı Güncellenemedi!!");
								git("ana.php?git=koseyazisiduzenle&id=$idsi","1");
						...
						hbaslik='$hbaslik',hetiket='$hetiket',hkisa='$hkisa',hicerik='$hicerik',videokod='$videokod',
						aktif='$haktif',mansetgoster='$mansetgoster',anasayfagoster='$anasayfagoster',
						ustmanset='$ustmanset',sondakika='$sondakika',flash='$flash',yorumakapa='$yorumkapa',hmanset='$yuklenenresim',koseid='$koseyaz' where hid='$idsi'");
							if($haberekle){
								uyari("Köşe Yazısı Başarıyla Güncellendi!!");
								git("ana.php?git=koseyazisiduzenle&id=$idsi","1");
							}else{
                ...
                <div class="widgetcontent bordered shadowed nopadding">
                    <form class="stdform stdform2" method="post" action="" enctype="multipart/form-data">
						...
						?>
							...
							<p>
                                <label>Kose Yazari</label>
                                <span class="field"><select name="koseyaz" id="selection2" class="uniformselect">
								<?php 
								$koseyazarcek	=mysql_query("select * from koseyazarlari");
									while ($cekk=mysql_fetch_assoc($koseyazarcek)){
									
										$yazaradi	=$cekk["adisoyadi"];
										$yazid		=$cekk["kosid"];
								?>
							<option value="<?php echo $yazid;?>" <?php if($yazid==$koselid){ echo "selected";} ?>><?php echo $yazaradi;?></option>
                                <?php
								}
								?>
